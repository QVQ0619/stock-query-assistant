...
system_prompt = """我是股票查询助手，以下是关于股票历史价格表 stock_price 的字段，我可能会编写对应的 SQL，对数据进行查询
-- 股票历史价格表
...
class TavilySearchTool(BaseTool):
    description = '使用 Tavily 搜索引擎进行互联网搜索，获取实时信息和最新资讯'
    parameters = [
        {
            'name': 'query',
            'type': 'string',
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        if not TAVILY_API_KEY:
            return "未配置 Tavily API 密钥，无法进行联网搜索。请在系统环境变量中设置 TAVILY_API_KEY。"
...
class ExcSQLTool(BaseTool):
    description = '对于生成的 SQL，进行 SQL 查询，并自动可视化'
    parameters = [
        {
            'name': 'sql_input',
            'type': 'string',
        ...
        },
        {
            'name': 'need_visualize',
            'type': 'boolean',
            'description': '是否需要可视化和统计信息，默认 True。如果是对比分析等场景可设为 False，不进行可视化。',
            'required': False,
            'default': True
        }
    ]
    def call(self, params: str, **kwargs) -> str:
        ...
        database = args.get('database', 'bi')
        engine = create_engine(
            f"mysql+mysqlconnector://root:root@localhost:3306/{database}?charset=utf8mb4",
...
class ArimaStockTool(BaseTool):
    description = '对指定股票 (ts_code) 的收盘价进行 ARIMA(5,1,5) 建模，并预测未来 n 天的价格，返回预测表格和折线图。'
    parameters = [
        {
            'name': 'ts_code',
            'type': 'string',
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        # 连接本地 MySQL，获取历史收盘价
        engine = create_engine(
            f"mysql+mysqlconnector://root:root@localhost:3306/bi?charset=utf8mb4",
            connect_args={'connect_timeout': 10}, pool_size=10, max_overflow=20
        )
        sql = f"""
        ...
        try:
            ...
            plt.close()
            img_path = os.path.join('image_show', filename)
            img_md = f'![ARIMA 预测]({img_path})'
            return f"{pred_df.to_markdown(index=False)}\n\n{img_md}"
        except Exception as e:
            return f'ARIMA 建模或预测出错：{str(e)}'

# boll_detection 工具
@register_tool('boll_detection')
class BollDetectionTool(BaseTool):
    description = '对指定股票 (ts_code) 的收盘价进行布林带异常点检测，默认检测过去 1 年，也可自定义时间范围，返回超买和超卖日期及布林带图。'
    parameters = [
        {
            'name': 'ts_code',
            'type': 'string',
        ...
        },
        {
            'name': 'start_date',
            'type': 'string',
            'description': '检测起始日期，格式 YYYY-MM-DD，选填',
            'required': False
        },
        {
            'name': 'end_date',
            'type': 'string',
            'description': '检测结束日期，格式 YYYY-MM-DD，选填',
            'required': False
        }
    ]
    def call(self, params: str, **kwargs) -> str:
...
class ProphetAnalysisTool(BaseTool):
    description = '对指定股票 (ts_code) 的收盘价进行 Prophet 周期性分析，分解 trend、weekly、yearly 并可视化，支持自定义时间范围。'
    parameters = [
        {
            'name': 'ts_code',
            'type': 'string',
        ...
        },
        {
            'name': 'start_date',
            'type': 'string',
            'description': '分析起始日期，格式 YYYY-MM-DD，选填',
            'required': False
        },
        {
            'name': 'end_date',
            'type': 'string',
            'description': '分析结束日期，格式 YYYY-MM-DD，选填',
            'required': False
        }
    ]
    def call(self, params: str, **kwargs) -> str:
        ...
        df = pd.read_sql(sql, engine)
        if len(df) < 30:
            return '历史数据不足，无法进行 Prophet 周期性分析。'
        ...
        try:
            ...
            plt.close(fig)
            img_path = os.path.join('image_show', filename)
            img_md = f'![Prophet 周期分解]({img_path})'
            return f"Prophet 周期性分解（趋势、周、年）：\n\n{img_md}"
        except Exception as e:
            return f'Prophet 建模或分解出错：{str(e)}'
...
def init_agent_service():
    ...
    # 自定义工具配置（包含 Tavily 搜索）
    tools = ['exc_sql', 'arima_stock', 'boll_detection', 'prophet_analysis', 'tavily_search']
...
def app_gui():
    try:
        ...
        chatbot_config = {
            'prompt.suggestions': [
                '查询 2024 年全年贵州茅台的收盘价走势',
                '统计 2024 年 4 月国泰君安的日均成交量',
                '对比 2024 年中芯国际和贵州茅台的涨跌幅',
                '预测贵州茅台未来 7 天的收盘价',
                '检测贵州茅台近一年超买超卖点',
                '分析贵州茅台近一年周期性规律',
                '贵州茅台最近的热点新闻',
